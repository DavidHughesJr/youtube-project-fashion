  ...
  return (
    <Container maxWidth='lg'>
      <Stack spacing={2}>
        ...
        <Typography mb={5} variant='h5'> Receipt Total: {orderReceipt.order_value.formatted_with_symbol} </Typography>
      </Stack>
      <Stack>
        {
          orderReceipt?.order?.line_items?.map((item) => (
            <Stack key={item.id} mb={5} flexDirection={{ xs: 'column', sm: 'row' }}
              gap={2} justifyContent='space-between' alignItems={{ xs: 'center', sm: "flex-start" }}
            >
              <Box width='150px'>
      ...
      <Stack mt={15} spacing={2} width={{ xs: '100%', md: '50%' }}>
        ...
        <Button component={Link} to='/catalogue' variant='contained'> Catalogue </Button>
      </Stack>
