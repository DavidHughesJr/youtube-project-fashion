...
import { Box, Stack, Typography, Button, ToggleButton,
CircularProgress, styled, ToggleButtonGroup } from "@mui/material"
  ...
  return (
    <Stack spacing={2} alignItems={{xs: 'center', md: 'flex-start'}}>
      ...
      >
        {
          sizes? sizes?.map((item) => (
            <ToggleButton key={item.id} sx={{border: '1px solid grey !important', borderRadius: '25px !imporant'}} value={item.id}>
              {item.name}
            </ToggleButton>
          )) : ''
        }
      ...
      </Stack>
      {
        ...
        <Button disabled={disableBuy} color="secondary" variant="contained"
